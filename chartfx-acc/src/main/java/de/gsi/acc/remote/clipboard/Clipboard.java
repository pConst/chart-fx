                  ...
                  ctx.uploadedFiles("clipboardData").forEach(file -> {
                      ...
                      if (exportName == null || exportName.isBlank() || !fileName.contains(".")) {
                          try {
                              ...
                              LOGGER.atError().setCause(e).addArgument(fileName).log("error while reading test image from '{}'");
                          }
                      ...
                      final String[] exportNameData = fileName.substring(p, fileName.length()).replace("/", "").split("\\.");
